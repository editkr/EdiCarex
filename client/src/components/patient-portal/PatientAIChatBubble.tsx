...
import { MessageSquare, X, Send, Sparkles, Loader2, Heart, Shield } from 'lucide-react'
...
export default function PatientAIChatBubble() {
    const { config } = useOrganization()
    const [isOpen, setIsOpen] = useState(false)
    const [messages, setMessages] = useState<any[]>([
        {
            role: 'assistant',
            content: 'Bienvenido a **EdiCarex Hospital**. Soy su Concierge de Salud Virtual. ¿En qué puedo asistirle hoy con respecto a sus citas, resultados o servicios médicos?',
            timestamp: new Date(),
        },
    ])
    ...
    const handleSend = async () => {
        ...
        const patientName = user.firstName ? `${user.firstName} ${user.lastName}` : 'Paciente'
        ...
        try {
            const response = await aiAPI.chat({
                message: input,
                context: `Patient portal chat assistant. User: ${patientName}. ID: ${user.id || 'Unknown'}`
            })
            ...
            console.error('AI Portal Chat Error:', error)
            setMessages(prev => [...prev, {
                role: 'assistant',
                content: 'Lo siento, he tenido un problema técnico. ¿Podrías intentar de nuevo o contactar con recepción?',
                timestamp: new Date(),
            }])
        } finally {
            setIsLoading(false)
    ...
    if (!config?.ai?.enabled || !config?.ai?.features?.patientConcierge) return null
                            ...
                            <div className="flex items-center gap-4 relative z-10">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-2 mt-1">
                                        ...
                                        <span className="text-[10px] font-bold text-cyan-50 uppercase tracking-[0.1em]">Health Assistant Active</span>
                                    </div>
                                </div>
                            </div>
                            <Button
                                                ...
                                                <>
                                                    ...
                                                    <div className="flex items-center gap-1.5 grayscale opacity-70">
                                                        ...
                                                        <span className="text-[9px] text-zinc-500 font-bold uppercase tracking-widest">
                                                            {MODEL_DISPLAY_NAMES[m.model] || m.model || 'Salud AI'}
                                                        </span>
                                                    </div>
                                                </>
                                            )}
                        ...
                        <div className="p-5 bg-white dark:bg-zinc-950 border-t border-zinc-100 dark:border-zinc-900 shadow-[0_-10px_40px_rgba(0,0,0,0.03)]">
                            ...
                            <div className="flex flex-col items-center gap-1 mt-3">
                                <div className="flex items-center gap-1.5">
                                    <Heart className="h-3 w-3 text-cyan-600/50" />
                                    <span className="text-[9px] text-zinc-500 font-bold uppercase tracking-[0.15em]">Soporte EdiCarex Hospital</span>
                                </div>
                            </div>
                        </div>
                    </motion.div>
