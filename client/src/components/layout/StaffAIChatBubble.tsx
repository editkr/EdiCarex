...
export default function StaffAIChatBubble() {
    const { config } = useOrganization()
    const [isOpen, setIsOpen] = useState(false)
    const [messages, setMessages] = useState<any[]>([
        {
            role: 'assistant',
            content: 'Sintonía establecida con el **Soporte Clínico EdiCarex**. Soy su consultor senior; puedo guiarle en los módulos de **Pacientes, Citas, Emergencia, Farmacia, Camas o Análisis de Datos**. ¿En qué puedo asistirle hoy?',
            timestamp: new Date(),
        },
    ])
    ...
    const handleSend = async () => {
        ...
        const staffName = user.firstName ? `${user.firstName} ${user.lastName}` : 'Colega'
        ...
        try {
            const response = await aiAPI.chat({
                message: input,
                context: `Medical Staff Assistant. User: ${staffName}. Role: ${user.role || 'Personnel'}`
            })
            ...
            console.error('AI Staff Chat Error:', error)
            setMessages(prev => [...prev, {
                role: 'assistant',
                content: 'Lo siento, colega. He tenido un problema técnico. ¿Podría intentar de nuevo?',
                timestamp: new Date(),
            }])
        } finally {
            setIsLoading(false)
    ...
    if (!config?.ai?.enabled || !config?.ai?.features?.staffAssistant) return null
                    ...
                    >
                            ...
                            <div className="flex items-center gap-4 relative z-10">
                                ...
                                <div>
                                    ...
                                    <div className="flex items-center gap-2 mt-1">
                                        ...
                                        <span className="text-[10px] font-bold text-cyan-50 uppercase tracking-[0.1em]">Core Inference Active</span>
                                    </div>
                                </div>
                            </div>
                            <Button
                                                ...
                                                <>
                                                    ...
                                                    <div className="flex items-center gap-1.5 grayscale opacity-70">
                                                        ...
                                                        <span className="text-[9px] text-zinc-500 font-bold uppercase tracking-widest">
                                                            {MODEL_DISPLAY_NAMES[m.model] || m.model || 'Inference'}
                                                        </span>
                                                    </div>
                                                </>
                                            )}
                        ...
                        <div className="p-5 bg-white dark:bg-zinc-950 border-t border-zinc-100 dark:border-zinc-900 shadow-[0_-10px_40px_rgba(0,0,0,0.03)]">
                            <div className="relative group">
                                ...
                                <Input
                                    placeholder="Consultar experto EdiCarex..."
                                    value={input}
                            ...
                            <div className="flex items-center justify-between mt-3 px-1">
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-[9px] text-zinc-500 font-bold uppercase tracking-[0.15em]">Admin Protocol</span>
                                </div>
                                <span className="text-[9px] text-zinc-400 font-medium italic">EdiCarex Enterprise v2.5</span>
                            </div>
                        </div>
                    </motion.div>
                )}
