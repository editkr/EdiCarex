...
import { vacunacionAPI, patientsAPI, healthStaffAPI } from '@/services/api'
...
export default function VacunacionPage() {
    ...
    // Personal disponible con isAvailable=true
                                    ...
                                    <div className="md:col-span-2 border-t border-border pt-4">
                                        <div className="flex gap-4">
                                            ...
                                            <div className="flex-1">
                                                ...
                                                <select
                                                    className="w-full bg-background border border-border rounded-md px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500"
                                                    value={form.appliedBy}
                                                    onChange={e => setForm(f => ({ ...f, appliedBy: e.target.value }))}
                                                >
                                                    <option value="">-- Seleccionar personal --</option>
                                                    {availableStaff.map((s: any) => (
                                                        <option key={s.id} value={`${s.profession?.replace(/_/g, ' ')} ${s.user?.firstName} ${s.user?.lastName}`}>
